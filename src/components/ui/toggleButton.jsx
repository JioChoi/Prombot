...
export default function ToggleButton({configKey, children, icon, invert=false}) {
    ...
    return (
        <div className={`px-2 py-[1px] rounded-sm text-md font-medium text-zinc-300 lg:hover:bg-zinc-700 select-none hover:cursor-pointer
            ${(invert ? !config[configKey] : config[configKey]) ? 'text-yellow-100' : 'text-zinc-300'}
            `}
            onClick={() => {
        ...
        >
            {
                (invert ? !config[configKey] : config[configKey]) ?
